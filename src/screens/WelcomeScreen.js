...
const WelcomeScreen = ({ navigation }) => {
  return (
    <View className='flex-1 bg-[#F7F8F9] px-5 justify-end mb-7'>
      ...
      <Button bgColor={'bg-[#6C25FF]'} textColor={'text-[#FFFFFF]'} title={'Create Account'} onPress={() => navigation.push('Register')} />
      <Button bgColor={'bg-[#6C25FF4B]'} textColor={'#1D2226'} title={'Already Registered? Login'} onPress={() => navigation.push('SignIn')} />
    </View>
  )
}
